...
const messageMap = new Map(); // mapa das mensagens do ponto
...
export async function voiceStateHandler(client, oldState, newState) {
  ...
  if (oldState.channel && oldState.channel.parentId === CATEGORY_VOICE_ID &&
    ...
    const totalMinutes = await atualizarHorasUsuario(userId, minutosTotais);
    ...
    const embed = new EmbedBuilder()
      ...
      .addFields(
        ...
        { name: 'TOTAL', value: `${totalMinutes}m`, inline: true },
        { name: '-#', value: 'O ponto foi fechado automaticamente, o membro saiu da call.' }
      );

    const msg = messageMap.get(userId);
