...
/**
 * atualizarHorasUsuario(userId, minutosDelta)
 * - minutosDelta: integer (positivo para adicionar, negativo para remover)
 * Returns: new total minutes (number) or null on error
 */
export async function atualizarHorasUsuario(userId, minutosDelta) {
  try {
    ...
    if (idx === -1) {
      // append new row: A=userId, B=nome(empty),C=idEmJogo(empty),D=login(empty),E=advertencia(0),F=minutos
    ...
    const absoluteRow = start + idx + 1; // sheet is 1-indexed
